...
def process_file(file_path):
    ...
    date = "Date not found"# Step 2: Import All the Required Libraries
import os
from langchain.schema import Document
from getpass import getpass
from langchain.vectorstores import Pinecone as LangChainPinecone  # Rename to avoid collision
...
if index_name not in pc.list_indexes().names():
    pc.create_index(
        ...
        replicas=1,
        shard_size=1,  # Use a smaller shard size if fewer documents are expected
        enable_vectorized_query=True,
        timeout=30,
        **ServerlessSpec(shard_size=1)  # Use ServerlessSpec for serverless deployment
    )
